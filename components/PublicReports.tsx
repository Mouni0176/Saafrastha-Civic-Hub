...
const ReportCard = memo(({ report, user, onVote }: { report: Report, user: User | null, onVote: (id: string, type: 'support' | 'dispute') => void }) => {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl transition-all group flex flex-col h-full">
      <div className="relative h-64 overflow-hidden bg-slate-100">
        {report.imageUrl ? (
          <img src={report.imageUrl} alt={report.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" loading="lazy" />
        ) : (
      ...
      <div className="p-8 flex-grow">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2 leading-tight line-clamp-1">{report.title}</h3>
      ...
      <div className="px-6 py-4 bg-slate-50 border-y border-slate-100 flex items-center justify-around gap-2">
        <button 
          ...
          className={`flex-1 py-3 px-4 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all border active:scale-95 ${
            hasSupported 
        ...
        >
          <ThumbsUp size={14} fill={hasSupported ? "currentColor" : "none"} /> 
        ...
        >
          <ThumbsDown size={14} fill={hasDisputed ? "currentColor" : "none"} /> 
          Dispute ({report.disputeCount})
        </button>
      </div>

      <div className="px-8 py-4 bg-white flex items-center justify-between border-t border-slate-50">
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="bg-white p-4 rounded-[2rem] border border-slate-200 shadow-sm mb-12 flex flex-col md:flex-row items-center justify-between gap-6">
           ...
           <div className="relative max-w-md w-full">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3 px-12 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 text-sm font-bold uppercase tracking-tight"
                value={searchTerm}
        ...
        {filteredReports.length === 0 && (
          <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
            ...
            <h3 className="text-xl font-bold text-slate-800 uppercase tracking-widest">No matching records</h3>
